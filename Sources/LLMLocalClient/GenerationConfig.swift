...
public struct GenerationConfig: Sendable {
    ...
    /// 新しい生成設定を作成します。
    /// - Parameters:
    ///   - maxTokens: 生成する最大トークン数。デフォルトは `nil`（コンテキスト上限まで）。
    ///   - temperature: サンプリング温度。デフォルトは 0.7。
    ///   - topP: Top-p サンプリングの閾値。デフォルトは 0.9。
    public init(maxTokens: Int? = nil, temperature: Float = 0.7, topP: Float = 0.9) {
        self.maxTokens = maxTokens
        self.temperature = temperature
        self.topP = topP
    }

    /// デフォルトの生成設定（maxTokens: nil, temperature: 0.7, topP: 0.9）。
